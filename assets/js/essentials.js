...
/* ---------------------------------------------------------------------- */
    ...
    $(".normal_menu .menu > ul > li.menu-dropdown-icon:before").click(function () {
	    if ($(window).width() <= 991) {
            $(this).children("ul").slideToggle(500);
            $(this).toggleClass("subnav-open");
	...
	var $toggleButton = $('.toggle-button'),
    	...
    	$FHhtml = $('html'),
    	$FHoverlay = $('.site_overlay'),
    	$FHpage = $("#page > div:not(#header-container)");
	// Hamburger button
	$toggleButton.on('click', function() {
		$(this).toggleClass('button-open');
		$menuWrap.toggleClass('menu-show');
		$FHhtml.toggleClass('full_menu');
		$FHoverlay.toggleClass('open',250);
	});
    ...
    //the following hides the menu when a click is registered outside
	$FHoverlay.on("click", function (){
        //if ($toggleButton.hasClass('button-open')){
            $toggleButton.removeClass('button-open');
			$menuWrap.removeClass('menu-show');
			$FHhtml.removeClass('full_menu');
			$FHoverlay.removeClass('open',250);
		//}
	});		

    //Full site hamburger menu
    $(".full_site_hamburger1 .menu > ul > li").hover(function() {
        //no more overlapping menus
        //hides other children menus when a list item with children menus is clicked
        var thisMenu = $(this).children("ul");
        var prevState = thisMenu.css('display');
    ...
    $(".full_site_hamburger1 .menu > ul > li").click(function () {
            $(this).toggleClass("subnav-open");
            $(this).children("ul").slideToggle(500);
    });
    $(".full_site_hamburger1 .menu > ul > li.menu-dropdown-icon:before").click(function () {
            $(this).children("ul").slideToggle(500);
            $(this).toggleClass("subnav-open");
            $(".menu > ul > li").toggleClass("subnav-open");
    });    
	...
	var $HamMenu = $('.hamburger-menu'),
    	$HamBody = $('body'),
    	$HamSiteover = $('.site_overlay');
	$HamMenu.click(function() {
		$(this).toggleClass('active',250);
		$HamBody.toggleClass("humnav-open");
		$HamSiteover.toggleClass('open',250);
	});
	...
	$Mobilehamburger.on("click", function (e){
		...
		if ($this.hasClass("active")){
			...
			$menu.removeClass("menu_open1").addClass("menu_close1").slideUp(350);
			...
			$menu.addClass("menu_open1").removeClass("menu_close1").slideDown(350);
		...
		$page.on("click", function (){
			...
			$menu.removeClass("menu_open1").addClass("menu_close1").slideUp(350);
			$header.removeClass("header_menu_open").addClass("header_menu_close");
			$('.site_overlay').removeClass('open',350);
		});		
	});
