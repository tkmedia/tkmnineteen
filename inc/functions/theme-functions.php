 ...
 * Schema for <body> tag.
 * --------------------------------------------------------------------------- */
if ( ! function_exists( 'tkmnineteen_schema_body' ) ) :
 ...
 if( ! class_exists( 'New_Walker_Category' ) )
{
	 ...
	 class dynamicWalkerNavMenu extends Walker_Nav_Menu
	{
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$item_output .= '<div class="child-wrap"><a'. $attributes .'>';
 ...
 * Add a dropdown icon to top-level menu items
 */
function tkmnineteen_nav_add_dropdown_icons( $output, $item, $depth, $args ) {

	// Only add class to 'top level' items on the 'primary' menu.
	if ( ! isset( $args->theme_location ) || 'primary1' !== $args->theme_location ) {
		return $output;
	}
	...
	if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
		...
		$icon = tkmnineteen_icon( array( 'icon' => 'navigate-down', 'size' => 16 ) );
...
add_filter( 'walker_nav_menu_start_el', 'tkmnineteen_nav_add_dropdown_icons', 10, 4 );
 ...
 */
function tkmnineteen_nav_menu_link_attributes( $atts, $item, $args, $depth ) {
...
add_filter( 'nav_menu_link_attributes', 'tkmnineteen_nav_menu_link_attributes', 10, 4 );

/* ---------------------------------------------------------------------------
 * Add html5shiv script to <head> conditionally for IE8 and under
 * --------------------------------------------------------------------------- */
function tkmnineteen_html5shiv() { ?>
	<!--[if lt IE 9]>
	<script src="<?php echo get_template_directory_uri(); ?>/js/html5shiv.min.js"></script>
	<![endif]-->
<?php }
add_action('wp_head', 'tkmnineteen_html5shiv');
 ...
 * --------------------------------------------------------------------------- */
if ( ! function_exists( 'tkmnineteen_content_nav' ) ):
function tkmnineteen_content_nav( $nav_id ) {
	global $wp_query;
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'tkmnineteen' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous Page', 'tkmnineteen' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next Page' , 'tkmnineteen' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Previous Pages', 'tkmnineteen' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'New Pages <span class="meta-nav">&rarr;</span>', 'tkmnineteen' ) ); ?></div>
		<?php endif; ?>
...
function tkmnineteen_gravityforms_domain( $notification, $form, $entry ) {
	if( $notification['name'] == 'Admin Notification' ) {
		$notification['message'] .= 'Sent from ' . home_url();
	}
	return $notification;
}
add_filter( 'gform_notification', 'tkmnineteen_gravityforms_domain', 10, 3 );
 ...
 * @return null
 * --------------------------------------------------------------------------- */
function tkmnineteen_pp( $obj, $label = '' ) {
	$data = json_encode( print_r( $obj,true ) );
	?>
	<style type="text/css">
		#bsdLogger {position: absolute;top: 30px;right: 0px;border-left: 4px solid #bbb;padding: 6px;background: white;color: #444;z-index: 999;font-size: 1.25em;width: 400px;height: 800px;overflow: scroll;}
	</style>
 ...
 * --------------------------------------------------------------------------- */
function tkmnineteen_attachment_id_on_images( $attr, $attachment ) {
	if( !strpos( $attr['class'], 'wp-image-' . $attachment->ID ) ) {
		$attr['class'] .= ' wp-image-' . $attachment->ID;
	}
	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'tkmnineteen_attachment_id_on_images', 10, 2 );

/* ---------------------------------------------------------------------------
 * Default Image Link is None
 * --------------------------------------------------------------------------- */
function tkmnineteen_default_image_link() {
	$link = get_option( 'image_default_link_type' );
	if( 'none' !== $link )
		update_option( 'image_default_link_type', 'none' );
}
add_action( 'init', 'tkmnineteen_default_image_link' );
...
add_action( 'admin_menu' , 'tkmnineteen_remove_post_custom_fields_now' );
...
function custom_field_excerpt() {
	...
	if ( '' != $text ) {
		...
		the_excerpt( sprintf(
			__( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'tkmnineteen' ),
 ...
 * --------------------------------------------------------------------------- */
function tkmnineteen_remove_caption_width( $current_html, $attr, $content ) {
...
add_filter( 'img_caption_shortcode', 'tkmnineteen_remove_caption_width', 10, 3 );
 ...
 * --------------------------------------------------------------------------- */
function tkmnineteen_lang_switcher() {
...
function languages_list_footer(){
    $languages = icl_get_languages( 'wpml_active_languages', NULL, 'skip_missing=0' );
    if(!empty($languages)){
        ...
        foreach($languages as $l){
            ...
            if(!$l['active']) echo '<a href="'.$l['url'].'">';
            if(!$l['active']) echo icl_disp_language($l['native_name'], $l['translated_name']);
            if(!$l['active']) echo '</a>';
            echo '</li>';
        }
        echo '</ul></div>';
...
function tkmnineteen_lang_switcher2() {
	define( 'ICL_DONT_LOAD_NAVIGATION_CSS', true );
	define( 'ICL_DONT_LOAD_LANGUAGE_SELECTOR_CSS', true );
	define( 'ICL_DONT_LOAD_LANGUAGES_JS', true );
	$lang = icl_get_languages( 'wpml_active_languages', NULL, 'skip_missing=0' );
	//$lang = apply_filters( 'wpml_active_languages', NULL, 'skip_missing=0' );
	if ( count( $lang ) > 1 ) {
		$html = '<div class="theme-lang-switcher">';
		foreach( $lang as $value ) {
			if ( 0 == $value[ 'active' ] ) {
				//$html .= '<li class="lang-item"><a class="theme-lang" href="' . esc_url( $value[ 'url' ] ) . '">' . esc_html( $value[ 'native_name' ] )  . '</a></li>';
				// flag with native name
				$html .= '<li class="menu-item"><a class="theme-lang" href="' . esc_url( $value[ 'url' ] ) . '"><img src="'.$value['country_flag_url'].'" height="12" alt="'.$value['language_code'].'" width="18" />' . esc_html( $value[ 'native_name' ] ) . '</a></li>';
				//only flag
				//$html .= '<li class="menu-item menu-item-language"><a href="' . $value['url'] . '"><img src="' . $value['country_flag_url'] . '" height="12" alt="' . $value['language_code'] . '" width="18" /></a></li>';
			}
		}
		$html .= '</div><!-- end .theme-lang-switcher -->';
		return apply_filters( 'theme_lang_switcher_html', $html, $lang );
...
// TO ADD our function to the post content filter - uncheck below
//add_action( 'the_content', 'wpb_author_info_box' );

// Allow HTML in author bio section
remove_filter('pre_user_description', 'wp_filter_kses');

// Change flexible content layout title Label
...
//}
// name
//add_filter('acf/fields/flexible_content/layout_title/name=flex_con', 'my_acf_flexible_content_layout_title', 10, 4);
...
function fb_move_admin_bar() { ?>
	<style type="text/css">
		...
		body.logged-in.admin-bar #wphead {
			padding-top: 0;
		}
		...
		body.logged-in #wpadminbar .quicklinks .menupop ul {
			bottom: 28px;
		}
	</style>
<?php }
